...
import com.app.codemasterpiecebackend.domain.dto.post.PostTocDTO;
...
public class MarkdownUtil {
    ...
    public static List<PostTocDTO> extractToc(String markdown) {
        if (markdown == null || markdown.isBlank()) return new ArrayList<>();
        String safeMarkdown = preprocessMath(markdown).replaceAll("<br\\s*/?>", "  \n");
        Node document = PARSER.parse(safeMarkdown);
        List<PostTocDTO> tocList = new ArrayList<>();
        Map<String, Integer> slugCounts = new HashMap<>();
        extractTocRecursively(document, tocList, slugCounts);
        return tocList;
    }

    private static void extractTocRecursively(Node node, List<PostTocDTO> tocList, Map<String, Integer> slugCounts) {
        if (node instanceof Heading heading) {
            ...
            slugCounts.put(baseSlug, count + 1);
            String id = (count == 0) ? baseSlug : baseSlug + "-" + count;
            tocList.add(new PostTocDTO(id, rawText, heading.getLevel()));
        }
